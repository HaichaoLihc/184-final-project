...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //
  const double sensor_x = (2.0 * x - 1.0) * tan(radians(hFov) / 2.0);
  const double sensor_y = (2.0 * y - 1.0) * tan(radians(vFov) / 2.0);

  Vector3D dir = c2w * Vector3D(sensor_x, sensor_y, -1.0).unit();
  dir.normalize();
